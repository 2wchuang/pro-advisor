...
| | 载荷 | 回复 | 伪造标记 |
| --- | --- | --- | --- |
| 旧镜像（0.2.5） | 667,925 字符 | 15,593 字符 | `[Tool result (` ×7、`[Assistant tool calls]` ×6、`[Assistant]:` ×4 |
| **brief（0.3.0）** | **2,533 字符** | 689 字符 | **无** |

载荷缩小 **264×**。（回复里出现过一次 `[Assistant]:`，核实后是**误报** —— 那是 advisor 在*讨论*这些标记本身，不是在*发出*它们；伴随标记 `[Tool result (` / `[Assistant tool calls]:` 均为 0。真伪造必然同时出现伴随标记。）

### 未决：核查能力的代价（已咨询 advisor，暂缓实现）

移除转录后，advisor 只能看到执行器**声称**的证据，看不到原始工具输出/代码原文。它在旧设计里可以指出“你引用的那行代码其实不是这个逻辑”。新设计用 `evidence` 字段缓解，但那是执行器筛选过的。

**已咨询 advisor 的判断（原文要点）**：保留当前载荷形状，**不要**恢复全量镜像；将来若需核查能力，应加**受限、可溯源的 artifact 通道**（`evidence` 分为 `claims` = 自述不可核验 / `artifacts` = 宿主从既有工具结果原样截取，带来源 id、cwd/exit code 或 file/revision/line range、截断状态），而不是恢复镜像。

**它同时纠正了我的推理**，这一点值得记下：

> 不要用“不曾测量”推断能力不重要，也不要仅因另一侧有数字就优先它。

我当时的 `unsure` 写的是“我用一个有实测的量（伪造率）换掉了一个只有定性描述的能力（核查）”，实际上是在用“有数据的一方优先”。这是错的 —— **测量不对称不是决策依据**。

**决定（用户）**：先不建 artifact 通道，**先测量**。理由与 advisor 自己的建议一致：它列出的判断条件是「观察到原始证据经常改变建议或抓到误读」，而这个频率**目前为零测量**。用未测量的收益去建一个新子系统，与 I-5/I-7/I-8 的决策原则（不为未证实的风险/收益引入复杂度）冲突。

**取舍反转的条件（advisor 给出，记录待用）**：任务结论高度依赖精确代码或命令输出、错误后果高，且**观察到**原始证据经常改变建议或抓到误读。满足时启用受限 artifact 通道，仍不恢复全量镜像。
